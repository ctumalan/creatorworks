...
function shareFrame(title, copy, body, nextLabel = "Continue") {
  ...
  return `<div class="share-layout${state.creatorStep === 0 ? " share-layout-intro" : ""}">${visual}<div class="share-work"><div class="share-progress"><span style="width:${progress}%"></span></div><p class="eyebrow">Your private workspace · ${state.creatorStep + 1} of 6</p><h1>${title}</h1><p class="share-copy">${copy}</p>${body}<div class="form-actions${state.creatorStep === 0 ? " share-start-actions" : ""}">${state.creatorStep > 0 ? `<button class="secondary-button" data-share-back>Back</button>` : ""}<button class="primary-button" data-share-next>${nextLabel}</button>${state.creatorStep === 0 ? `<button type="button" class="share-browse-link" data-route="discover">Browse other projects</button>` : ""}</div><p class="privacy-note">🔒 Private until you choose otherwise. Your work remains yours.</p></div></div>`;
}
