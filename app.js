...
const creators = [
  {
    ...
    initials: "CT",
    label: "Musician · Founder of CreatorWorks",
...
creators.push({ slug: "creatorworks-studio", name: "CreatorWorks Studio", initials: "CW",
  label: "In-house creator · Founded by Christian Tumalán",
  bio: "Our launch collection of practical tools, built in-house at CreatorWorks. Try something useful and tell us what worked, what confused you, and what would make it better." });
...
const studioVerification = 'Founder-confirmed: Christian Tumalán confirmed control of CreatorWorks Studio and its 11 listed projects on September 4, 2026. This is not independent verification or a guarantee of product quality.';
...
function listingSettingsPage() {
  ...
  const name = esc(state.session.user?.displayName || 'a CreatorWorks member');
...
function serverAccountPage() {
  ...
  if (!session) return `<section class="account-hero"><h1>Your CreatorWorks</h1><p>Checking your sign-in…</p></section>`;
  if (!session.authenticated) return `<section class="account-hero"><p class="eyebrow">My CreatorWorks</p><h1>Use things. Make things. Or both.</h1><p>One account for the work you discover and the things you create.</p><div class="account-actions"><a class="primary-button" href="/auth/sign-in?signup=1">Create my account</a><a class="secondary-button" href="/auth/sign-in">Sign in</a></div>${session.authReady ? '' : '<p>Sign-in is being connected. You can browse the projects now.</p>'}</section>`;
  const person = session.user;
  return `<section class="page-shell feedback-page"><div class="feedback-card">${session.isAdmin ? '<p><a class="secondary-button" href="/admin">Administration →</a></p>' : ""}<p class="eyebrow">Your CreatorWorks</p><h1>Welcome, ${esc(person.displayName)}.</h1>${session.databaseReady ? `<form data-profile-form><div class="feedback-identity"><label>Your public name<input name="displayName" value="${esc(person.displayName)}" maxlength="60" required></label><label>How you describe yourself<input name="label" value="${esc(person.label || '')}" maxlength="60" placeholder="Musician, Parent, Engineer…"></label></div><label>A little about you<textarea name="bio" maxlength="500">${esc(person.bio || '')}</textarea></label><label class="profile-visibility"><input type="checkbox" name="isPublic" ${person.isPublic ? 'checked' : ''}> Make my profile public</label><p data-profile-status role="status"></p><button class="primary-button" type="submit">Save my profile</button></form>` : '<p>You are signed in. Profile saving is being connected; changes are not available yet.</p>'}<form action="/auth/sign-out" method="post"><button class="secondary-button" type="submit">Sign out</button></form><p>Not your account? Sign out first, then sign in or create an account with your own email.</p></div></section>`;
}
...
function render() {
  ...
  if (window.CW_SERVER) {
    ...
    profile.setAttribute('aria-label', state.session?.authenticated ? 'My account' : 'Sign in to CreatorWorks');
...
document.addEventListener("click", async event => {
  ...
  if (event.target.closest("[data-feedback-submit]")) {
    ...
    state.communityPosts.unshift({ id: String(Date.now()), projectSlug: state.selected.slug, author, initials, label: card.querySelector("[data-feedback-label]").value.trim() || "CreatorWorks participant", response: response || signals[0], signals, createdAt: "Just now" });
...
function hydrateCatalog(list) {
  ...
  list.forEach((p, index) => {
    ...
    if (p.creator && (p.creator.slug || p.creator.name)) {
      ...
      const data = { slug, avatar: p.creator.avatar || '', name: p.creator.name, initials: p.creator.initials || String(p.creator.name || 'C').slice(0, 2).toUpperCase(), label: p.creator.label || 'CreatorWorks creator', bio: p.creator.bio || '', verified: !!p.creator.verified };
