...
export const GET: APIRoute = async ({ params }) => {
  ...
  return new Response(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(person.display_name)} · CreatorWorks</title><link rel="stylesheet" href="/styles.css"></head><body><header class="site-header"><a class="brand" href="/">CreatorWorks</a><a href="/dashboard" data-account-nav>My account</a></header><main id="app" class="page-shell"><section class="profile-hero"><span class="person-avatar large">${person.avatar_path ? `<img src="${escape(person.avatar_path)}" alt="">` : escape(initials)}</span><div><p class="eyebrow">CreatorWorks member</p><h1>${escape(person.display_name)}</h1><strong>${escape(person.identity_label)}</strong><p>${escape(person.bio)}</p>${person.website?`<a href="${escape(person.website)}" rel="noopener noreferrer nofollow" target="_blank">Website ↗</a>`:''}${person.verified?'<p>Creator identity reviewed by CreatorWorks</p>':''}</div></section>${work}</main><script src="/account-nav.js" defer></script></body></html>`, { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' } });
};
