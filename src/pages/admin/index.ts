...
</style></head><body><main class="admin-shell"><header class="admin-top"><strong>TryMyBuild · Administration</strong><a href="/admin/workspace">Administration home →</a></header>${section ? `<nav class="admin-nav" aria-label="Administration">${sections.map((key,i)=>`<a href="/admin?section=${key}" ${key === section ? 'aria-current="page"' : ''}>${labels[i]}</a>`).join('')}</nav>` : ''}${content}</main></body></html>`, { status, headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'private, no-store', 'X-Robots-Tag': 'noindex, nofollow' } });
}
