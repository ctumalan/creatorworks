...
function page(content: string, section: string, status = 200) {
  return new Response(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta name="robots" content="noindex,nofollow"><title>Administration · TryMyBuild</title><link rel="stylesheet" href="/styles.css"><style>
...
</style><link rel="icon" type="image/svg+xml" href="/assets/brand/trymybuild-favicon.svg"><link rel="apple-touch-icon" sizes="180x180" href="/assets/brand/trymybuild-app-180.png"></head><body><main class="admin-shell"><header class="admin-top"><strong>TryMyBuild · Administration</strong><a href="/admin/workspace">Administration home →</a></header>${section ? `<nav class="admin-nav" aria-label="Administration">${sections.map((key,i)=>`<a href="/admin?section=${key}" ${key === section ? 'aria-current="page"' : ''}>${labels[i]}</a>`).join('')}</nav>` : ''}${content}</main></body></html>`, { status, headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'private, no-store', 'X-Robots-Tag': 'noindex, nofollow' } });
}
