...
export const GET:APIRoute = async context => {
 ...
 const form=isOwner?'<section class="cw-panel"><h1>Invite an honest conversation.</h1><p>This is your project’s feedback page. Share its address with someone who might use it.</p><a class="primary-button" href="/dashboard?view=creator">Open my creator inbox →</a><p class="cw-meta">Creators don’t submit reviews of their own projects.</p></section>':existingRow?`<section class="cw-panel"><h2>Your conversation is already started.</h2><p>Add an update or see the creator’s reply.</p><a class="primary-button" href="/dashboard/thread/${e(existingRow.id)}">Open my conversation →</a></section>`:`<section class="cw-panel"><h1>Tell the creator.</h1><p>What worked? What would make it better?</p>${!ready||!ownerReady?'<p class="cw-notice">This feedback inbox is being connected. You can try the project now; sending feedback isn’t available yet.</p>':''}<form action="/api/feedback" method="post"><input type="hidden" name="slug" value="${e(slug)}"><fieldset><legend>1. Did it help you?</legend>${choices('helpful',helpfulChoices)}</fieldset><fieldset><legend>2. How did the price feel?</legend>${choices('price',priceChoices)}</fieldset><label for="message">What should the creator know?</label><textarea id="message" name="message" maxlength="800" required data-community-field data-counter-id="feedback-word-count" aria-describedby="feedback-conduct feedback-word-count" placeholder="What helped, what got in your way, or what would make it worth paying for…"></textarea><p id="feedback-conduct" class="comment-conduct"><strong>Be thoughtful. Be respectful.</strong> Discuss the project, not the person. Share constructive observations using clear, considerate language. Insults, harassment, and abusive wording aren’t welcome. Honest disagreement is. <a href="/community-guidelines">Guidelines</a></p><small id="feedback-word-count" class="word-counter">0 / 7 words minimum</small><p class="cw-meta">Qualifying feedback on another creator’s project earns one feedback credit. Help five different projects to unlock another project slot.</p><fieldset><legend>3. Who can see your feedback?</legend>${choices('visibility',{private:'Just me and the creator',public:'Share with the community'})}<p class="cw-meta">Public feedback includes your display name and appears after review. Private feedback is never included in public reviews. Administrators may access records for safety and support.</p></fieldset>${!user?`<p class="cw-meta">Sign in before answering so your feedback stays connected to you.</p><a class="primary-button" href="/auth/sign-in?next=${encodeURIComponent('/tell/'+slug)}">Sign in to give feedback</a>`:`<button class="primary-button" type="submit" ${!ready||!ownerReady?'disabled':''}>Send to the creator →</button>`}</form></section>`;
