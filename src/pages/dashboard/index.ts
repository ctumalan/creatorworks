...
export const GET:APIRoute=async context=>{
 ...
 try{
  ...
  if(result.error)feedback='<p class="cw-notice">Your feedback inbox is being connected. No feedback has been loaded yet.</p>';
   ...
   feedback=result.data.length?result.data.map(row=>feedbackCard(row,(creator?selected!.title:titleMap.get(row.project_slug))||row.project_slug,profiles.data?.find(p=>p.user_id===row.author_user_id)?.display_name||'Member',true,profiles.data?.find(p=>p.user_id===row.author_user_id)?.avatar_path||'')).join(''):`<section class="cw-panel"><h2>${creator?'Your next insight starts with a conversation.':'You haven’t sent feedback yet.'}</h2><p>${creator?'Share the project’s “Tell the creator” link. No invitation is required to participate.':'Try a project, then tell its creator what worked for you.'}</p><a href="${creator?'/tell/'+selected!.slug:'/'}">${creator?'Open feedback page →':'Find something useful →'}</a></section>`;
