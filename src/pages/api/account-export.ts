...
export const GET:APIRoute=async context=>{
 try{
  ...
  const read=async(table:string,key:string)=>{let rows:any[]=[];for(let page=0;;page++){const order=['profiles','account_preferences','account_deletion_requests'].includes(table)?'user_id':table==='category_engagement'?'updated_at':'created_at';const r=await m.db.from(table).select('*').eq(key,m.member.id).order(order).range(page*500,page*500+499);if(r.error)throw r.error;rows.push(...r.data);if(r.data.length<500)return rows;}};
