export function communityProgress(credit:{balance:number;towardNext:number;eligible:number}){
 ...
 <div class="slot-progress-footer"><a class="primary-button" href="/dashboard/community#review-projects">${completed?'Help the next creator':'Find a project to review'}</a><span><strong>${credit.balance}</strong> feedback ${credit.balance===1?'credit':'credits'} available · <a href="/dashboard/community#credit-rules">How credits work</a></span></div>
