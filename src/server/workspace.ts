...
export function notice(context:APIContext) {
 ...
 if(error==='eligibility')return '<p class="cw-notice" role="alert">Your verification request was not confirmed. You need 50 lifetime earned credits and a published project. Check your progress below; if both requirements are met, try again or contact Help &amp; contact.</p>';
...
export function workspace(title:string,body:string,active:string,admin=false,identity?:{name:string,avatar?:string}) {
 ...
 return surface(title,`<header class="workspace-heading ${identity?'workspace-welcome':''}">${picture}<div><h1>${e(title)}</h1></div></header>${body}`,active,200,admin);
}
