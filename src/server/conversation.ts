...
export async function conversation(db:any,userId:string,id:string,url:URL) {
 ...
 return `<section data-conversation-content aria-label="Selected conversation"><a href="/projects/${e(f.project_slug)}">View project</a>${feedbackCard(f,project.title,name(f.author_user_id),false)}${report()}<p class="cw-meta">Credit status: ${e(qualification.data?.status||'Not assessed')}${qualified?` · ${rating?.total||1} total credits`:''}. ${e(qualification.data?.reason||'')}</p>${rating?.total>1?`<p class="cw-notice">${rating.total===5?'Useful':'Made a difference'} · ${e(rating.reason)}</p>`:''}
 ${userId===project.owner_user_id&&qualified&&(rating?.total||1)<10?`<details class="cw-panel"><summary>Recognize this feedback</summary><p>Your recognition is optional. The reviewer keeps their basic credit without it. Choose a reward based on the feedback’s usefulness, not whether it praises your app.</p><form method="post" action="/api/messages"><input type="hidden" name="action" value="rate"><input type="hidden" name="id" value="${e(id)}"><input type="hidden" name="revision" value="${rating?.revision||0}"><label>Impact<select name="total" required>${(rating?.total||1)<5?'<option value="5">Useful · 5 total credits</option>':''}<option value="10">Made a difference · 10 total credits</option></select></label><label>What did it help you improve?<textarea name="reason" minlength="10" maxlength="300" required></textarea></label><p><a href="/dashboard/community#credit-rules">Reward limits and rules</a></p><button class="primary-button">Recognize feedback</button></form></details>`:''}
