...
test('all community writing surfaces show conduct guidance and seven-word counters',async()=>{
 const sources=await Promise.all(['../app.js','../src/pages/tell/[slug].ts','../src/pages/dashboard/thread/[id].ts'].map(path=>readFile(new URL(path,import.meta.url),'utf8')));
 for(const source of sources){assert.match(source,/Be thoughtful\. Be respectful\./);assert.match(source,/7 words minimum/);assert.match(source,/community-guidelines/);}
});
